...
// ============================================================
// RPC / AGGREGATION HELPERS (admin: always fresh, no unstable_cache)
// ============================================================

/**
 * Pre-computes ALL standard timeframes (today, week, month, year)
 * in a single database RPC call so the client can switch filters
 ...
 */
async function getCustomStatsRange(
  startDate: string,
  endDate: string,
  prevStartDate: string,
  prevEndDate: string
 ...
 */
async function getTopArticles(startDate: string, endDate: string, limit = 5) {
  try {
 ...
 */
async function getTopAds(startDate: string, endDate: string, limit = 5) {
  try {
...
/**
 * Recent activities across articles, ads, and categories.
 */
...
export async function getDashboardStats(filters?: {
  ...
  if (isCustom) {
    // Custom date range → dedicated RPC / real aggregation
    stats = await getCustomStatsRange(startDate, endDate, prevStartDate, prevEndDate)
  } else {
    // Standard timeframe → one RPC that returns all period fields
    stats = await getAllTimeframes()
  }
  ...
  const [topArticles, topCat, topAds, recentActivities] = await Promise.all([
    getTopArticles(topStart, endDate, 5),
    isCustom ? getTopCategoriesForPeriod(startDate, endDate, 5) : getTopCategories(5),
    getTopAds(topStart, endDate, 5),
    getRecentActivities(),
  ])

  const topCategories = topCat.length > 0 ? topCat : await getTopCategories(5)
