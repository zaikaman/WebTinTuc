...
export const blocksToHtml = (blocks: any[]) => {
  ...
  const htmlList = blocks.map((block) => {
    if (block.type === 'paragraph') {
      ...
      // data-editor-* attrs: handled by PostEditorView click delegation (inline onclick is
      // stripped by document.execCommand('insertHTML') and blocked under some CSP setups).
      return `<div id="${wrapperId}" data-editor-media="true" class="my-4 relative group" contenteditable="false" style="max-width: ${width}; margin: 0 auto;">
  ...
  <button type="button" data-editor-action="delete" class="absolute top-2 right-2 hidden group-hover:flex items-center justify-center w-8 h-8 rounded-full bg-red-600 hover:bg-red-700 text-white shadow-md active:scale-95 transition-all z-30" title="Xóa hình ảnh">
  ...
  <div class="absolute bottom-2 left-1/2 -translate-x-1/2 hidden group-hover:flex items-center gap-1.5 bg-black/85 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10 shadow-lg text-[11px] text-white font-bold select-none z-30 w-max min-w-max whitespace-nowrap">
    <button type="button" data-editor-action="resize" data-width="25%" class="hover:text-[#E55956] transition-colors px-1.5 py-0.5">25%</button>
    <span class="w-[1px] h-3 bg-white/20"></span>
    <button type="button" data-editor-action="resize" data-width="50%" class="hover:text-[#E55956] transition-colors px-1.5 py-0.5">50%</button>
    <span class="w-[1px] h-3 bg-white/20"></span>
    <button type="button" data-editor-action="resize" data-width="75%" class="hover:text-[#E55956] transition-colors px-1.5 py-0.5">75%</button>
    <span class="w-[1px] h-3 bg-white/20"></span>
    <button type="button" data-editor-action="resize" data-width="100%" class="hover:text-[#E55956] transition-colors px-1.5 py-0.5">100%</button>
    <span class="w-[1px] h-3 bg-white/20"></span>
    <button type="button" data-editor-action="crop" class="hover:text-[#E55956] transition-colors px-1.5 py-0.5 flex items-center gap-1">
      ...
      const width = block.width || '100%';
      return `<div data-editor-media="true" class="my-4 relative group" contenteditable="false" style="max-width: ${width}; margin: 0 auto;">
  <video controls src="${block.src || ''}" class="w-full max-h-[400px] rounded-xl border border-gray-200 shadow-sm"></video>
  <button type="button" data-editor-action="delete" class="absolute top-2 right-2 hidden group-hover:flex items-center justify-center w-8 h-8 rounded-full bg-red-600 hover:bg-red-700 text-white shadow-md active:scale-95 transition-all z-30" title="Xóa video">
    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
  </button>
  <div class="absolute bottom-2 left-1/2 -translate-x-1/2 hidden group-hover:flex items-center gap-1.5 bg-black/85 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10 shadow-lg text-[11px] text-white font-bold select-none z-30 w-max min-w-max whitespace-nowrap">
    <button type="button" data-editor-action="resize" data-width="25%" class="hover:text-[#E55956] transition-colors px-1.5 py-0.5">25%</button>
  ...
  <button type="button" data-editor-action="delete" class="absolute top-2 right-2 hidden group-hover:flex items-center justify-center w-8 h-8 rounded-full bg-red-600 hover:bg-red-700 text-white shadow-md active:scale-95 transition-all z-30" title="Xóa video nhúng">
    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
  </button>
  <div class="absolute bottom-2 left-1/2 -translate-x-1/2 hidden group-hover:flex items-center gap-1.5 bg-black/85 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10 shadow-lg text-[11px] text-white font-bold select-none z-30 w-max min-w-max whitespace-nowrap">
    <button type="button" data-editor-action="resize" data-width="25%" class="hover:text-[#E55956] transition-colors px-1.5 py-0.5">25%</button>
